...
public class ImageAlignmentController {
	...
	ImageAlignment get(@PathVariable String id) {
		...
		if (imagealignment == null) {
			throw new CustomNotFoundException(
					"An imagealignment with this ID does not exist or you dont have permissions to access it.");
		}
	...
	ImageAlignment add(@RequestBody @Valid ImageAlignment imagealignment, BindingResult result) {
		...
		if (result.hasErrors()) {
			...
			throw new CustomBadRequestException(
					"ImageAlignment is invalid. Missing required fields?");
		}
		if (imagealignment.getId() != null) {
			throw new CustomBadRequestException(
					"The imagealignment you want to add must not have an ID. The ID will be autogenerated.");
		}
		if (imagealignmentService.findByName(imagealignment.getName()) != null) {
			throw new CustomBadRequestException(
					"An imagealignment with this name already exists. ImageAlignment names are unique.");
		}
	...
	void update(@PathVariable String id, @RequestBody @Valid ImageAlignment imagealignment,
		...
		if (!id.equals(imagealignment.getId())) {
			throw new CustomBadRequestException(
					"ImageAlignment ID in request URL does not match ID in content body.");
		} else if (imagealignmentService.find(id) == null) {
			throw new CustomBadRequestException(
					"An ImageAlignment with this ID does not exist or you don't have permissions to access it.");
		} else {
