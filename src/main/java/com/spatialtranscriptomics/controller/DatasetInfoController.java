...
/**
 * This class is Spring MVC controller class for the API endpoint "rest/datasetinfo".
...
public class DatasetInfoController {
    ...
    /**
     * GET|HEAD /datasetinfo/ 
     * GET|HEAD /datasetinfo/?account={accountId}
    ...
    public @ResponseBody List<DatasetInfo> list(
        ...
        if (datasetinfos == null) {
            ...
            throw new CustomNotFoundException("No DatasetInfos found or you don't "
    ...
    public @ResponseBody DatasetInfo get(@PathVariable String id) {
        
        DatasetInfo datasetinfo = datasetinfoService.find(id);
        if (datasetinfo == null) {
            logger.info("Failed to reeturn dataset info " + id);
            throw new CustomNotFoundException("A DatasetInfo with ID " + id + 
    ...
    public @ResponseBody DatasetInfo add(
            @RequestBody @Valid DatasetInfo datasetinfo, 
        ...
        if (datasetinfo.getId() != null) {
            ...
            throw new CustomBadRequestException("The DatasetInfo you want to add must not have an ID. "
    ...
    public @ResponseBody void update(
        ...
        if (!id.equals(datasetinfo.getId())) {
            ...
            throw new CustomBadRequestException(
                    "A DatasetInfo with ID " + id + " does not exist or you "
    ...
    CustomInternalServerErrorResponse handleRuntimeException(RuntimeException ex) {
        logger.error("Unknown error in datasetinfo controller: " + ex.getMessage());
        return new CustomInternalServerErrorResponse(ex.getMessage());
    }
}
