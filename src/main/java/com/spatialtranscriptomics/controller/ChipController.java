...
public class ChipController {
	...
	List<Chip> list() {
		...
		if (chips == null) {
			throw new CustomNotFoundException(
					"No chips found or you dont have permissions to access them.");
		}
		return chips;
	}
	...
	Chip get(@PathVariable String id) {
		...
		if (chip == null) {
			throw new CustomNotFoundException(
					"A chip with this ID does not exist or you dont have permissions to access it.");
		}
		return chip;
	}
	...
	Chip add(@RequestBody @Valid Chip chip, BindingResult result) {
		...
		if (result.hasErrors()) {
			throw new CustomBadRequestException(
					"Chip is invalid. Missing required fields?");
		}
		if (chip.getId() != null) {
			throw new CustomBadRequestException(
					"The chip you want to add must not have an ID. The ID will be autogenerated.");
		} else if (chipService.findByName(chip.getName()) != null) {
			throw new CustomBadRequestException(
					"A chip with this name exists already. Chip names are unique.");
		}
	...
	void update(@PathVariable String id, @RequestBody @Valid Chip chip, BindingResult result) {
		...
		if (!id.equals(chip.getId())) {
			throw new CustomBadRequestException(
					"Chip ID in request URL does not match ID in content body.");
		} else if (chipService.find(id) == null) {
			throw new CustomBadRequestException(
					"A chip with this ID does not exist or you don't have permissions to access it.");
		} else {
