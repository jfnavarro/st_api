...
public class ChipController {
    ...
    HttpEntity<Chip> get(@PathVariable String id) {
        ...
        logger.info("Returning chips " + id);
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);
        headers.add("Cache-Control", "public");
        headers.add("Last-modified", DateOperations.RFC1123_DATE_TIME_FORMATTER.print(chip.getLast_modified()));
        return new HttpEntity<Chip>(chip, headers);
    }

    /**
     * GET /chip/lastmodified/{id}
     *
